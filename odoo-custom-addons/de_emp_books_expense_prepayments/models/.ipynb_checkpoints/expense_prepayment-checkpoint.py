...
class HRExpensePrepayment(models.Model):
    ...
    date = fields.Date(string='Requested Date', required=True, default=fields.Date.context_today, readonly=True, states={'draft': [('readonly', False)]},)

    accounting_date = fields.Date(readonly=True, states={'draft': [('readonly', False)], 'submit': [('readonly', False)], 'refused': [('readonly', False)]}, default=fields.Date.context_today, string="Accounting Date")
    ...
    description = fields.Text('Notes...', readonly=True, states={'draft': [('readonly', False)], 'submit': [('readonly', False)], 'refused': [('readonly', False)]})
    
    address_id = fields.Many2one('res.partner', compute='_compute_from_employee_id', store=True, readonly=False, copy=True, string="Employee Home Address", check_company=True, states={'done': [('readonly', True)], 'reconcile': [('readonly', True)]})
...
class HRExpensePrepaymentLine(models.Model):
    ...
    name = fields.Char('Description', compute='_compute_name', store=True, required=True, copy=True, states={'draft': [('readonly', False)], 'submit': [('readonly', False)], 'refused': [('readonly', False)]})
